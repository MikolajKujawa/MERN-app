...
const getPlacesByUserId = async (req, res, next) => {
    ...
    try {
        ...
        const error = new HttpError(
            'Something went wrong, could not find a place with that user id', 500
        );
        return next(error);
    }
    ...
    if(!places || places.length === 0) {
        const error = new HttpError(
            'Could not find a places for the provided id.', 404
        );
        return next(error);
    }
...
const deletePlace = async (req, res, next) => {
    ...
    try {
        ...
        const error = new HttpError(
            'Something went wrong, ccould not delete place.', 500
        );
        return next(error);
    }
